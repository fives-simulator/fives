...
int run_simulation(int argc, char **argv) {
  ...
  const std::chrono::time_point<std::chrono::steady_clock> chrono_start =
  ...
  if (argc < 4) {
    ...
    std::cout << "# USAGE: " << argv[0]
              << " <config file> <job file> <experiment_suffix>" << std::endl;
    std::cout << "#          [Both files are expected to be YAML]" << std::endl;
    std::cout << "# This program starts a WRENCH simulation of a batch "
                 "scheduler, with      "
              << std::endl;
    std::cout << "# emphasis on storage resources model and collecting "
                 "storage-related      "
              << std::endl;
    std::cout << "# metrics.                                                   "
                 "             "
  ...
  try {
    ...
    WRENCH_WARN("ERROR : Unable to load config due to bad type conversion : %s",
                e.what());
    return 1;
  } catch (const YAML::InvalidNode &e) {
    WRENCH_WARN(
        "ERROR : Unable to load config due to invalid config format : %s",
        e.what());
    return 1;
  }
  auto start = configFilename.find_last_of("/") + 1;
  configFilename =
      configFilename.substr(start, configFilename.find_last_of(".") - start);
  ...
  jobFilename =
      jobFilename.substr(start, jobFilename.find_last_of(".") - start);
  ...
  auto ctrl = simulation->add(
      new fives::Controller(batch_service, permanent_storage,
                            compound_storage_service, USER, jobs, config));

  /* Start Wrench simulation */
  WRENCH_INFO("Starting simulation...");
  const std::chrono::time_point<std::chrono::steady_clock> sim_start =
      std::chrono::steady_clock::now();
  simulation->launch();
  const std::chrono::time_point<std::chrono::steady_clock> sim_end =
      std::chrono::steady_clock::now();
  ...
  const std::chrono::time_point<std::chrono::steady_clock> chrono_end =
      std::chrono::steady_clock::now();

  std::cout << "##########################################" << std::endl;
  std::cout << "# Program duration : " << (chrono_end - chrono_start) / 1ms
            << "ms" << std::endl;
  std::cout << "# Sim duration : " << (sim_end - sim_start) / 1ms << "ms"
            << std::endl;
  std::cout << "# Trace processing duration : " << (chrono_end - sim_end) / 1ms
            << "ms" << std::endl;
  std::cout << "##########################################" << std::endl;
