...
namespace fives {
    ...
    std::vector<std::shared_ptr<wrench::FileLocation>> fives::LustreAllocator::operator()(const std::shared_ptr<wrench::DataFile> &file,
        ...
        if (this->lustreUseRR(ba_min_max)) {
            ...
            return this->lustreRRAllocator(file, resources, mapping, previous_allocations, stripe_count);
        } else {
            ...
            return this->lustreWeightedAllocator(file, resources, mapping, previous_allocations, stripe_count);
        }
    }

    ba_min_max LustreAllocator::lustreComputeMinMaxUtilization(const std::map<std::string, std::vector<std::shared_ptr<wrench::StorageService>>> &resources) const {
     ...
     */
    striping LustreAllocator::lustreComputeStriping(uint64_t file_size_b, size_t total_number_of_OSTs, unsigned int stripe_count) const {
    ...
    bool LustreAllocator::lustreOstIsUsed(const std::map<std::shared_ptr<wrench::DataFile>, std::vector<std::shared_ptr<wrench::FileLocation>>> &mapping, const std::shared_ptr<wrench::StorageService> ost) const {
    ...
    std::vector<std::shared_ptr<wrench::FileLocation>> LustreAllocator::lustreRRAllocator(
        ...
        auto current_striping = lustreComputeStriping(file->getSize(), rr_service_count, stripe_count);
    ...
    uint64_t LustreAllocator::lustreComputeOstPenalty(uint64_t free_space_b, uint64_t used_inode_count, double active_service_count) const {
    ...
    uint64_t LustreAllocator::lustreComputeOstWeight(uint64_t free_space_b, uint64_t used_inode_count, uint64_t ost_penalty) const {
    ...
    uint64_t LustreAllocator::lustreComputeOssPenalty(uint64_t free_space_b, uint64_t free_inode_count, size_t ost_count, size_t oss_count) const {
     ...
     *
     */
    std::vector<std::shared_ptr<wrench::FileLocation>> LustreAllocator::lustreWeightedAllocator(
        const std::shared_ptr<wrench::DataFile> &file,
        const std::map<std::string, std::vector<std::shared_ptr<wrench::StorageService>>> &resources,
        const std::map<std::shared_ptr<wrench::DataFile>, std::vector<std::shared_ptr<wrench::FileLocation>>> &mapping,
        const std::vector<std::shared_ptr<wrench::FileLocation>> &previous_allocations,
        ...
        auto striping = lustreComputeStriping(file_size_b, active_ost_count, stripe_count);
