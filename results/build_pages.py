...
def get_pipeline_jobs(session, pipeline_id):
    ...
    if not calibration_job or not analysis_job:
        raise RuntimeError(f"For pipeline {pipeline_id}, we're missing either the calibrated simulation job or the analysis job")
...
def get_job_artifacts(session, job, artifacts_archive_name):
    ...
    return download_file(session, f"{BASE_URL}/jobs/{job['id']}/artifacts", f"{ARTEFACTS_DIR}/{job['pipeline']['id']}/{artifacts_archive_name}.zip")
...
def build_previous_result(result_pages: list, pipeline_id: str):
    ...
    try:
        with open(f"./public/static/{pipeline_id}/results/exp_configurations/calibrated_config.yaml", "r", encoding='utf-8') as config:
    ...
    metrics = None
    try:
        with open(f"./public/static/{pipeline_id}/results/{pipeline_id}_metrics.yaml", "r", encoding='utf-8') as metrics:
