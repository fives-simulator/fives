...
void BasicFunctionalTest::create_platform_test() {
    ...
    ASSERT_EQ(link_names.size(), 1607); // Didn't check if the value is correct, this tests only tracks unplanned changes

    ASSERT_TRUE(simulation->doesLinkExist("backbone_permanent_storage"));
    ...
    ASSERT_TRUE(simulation->doesLinkExist("user42_down"));
    ASSERT_TRUE(simulation->doesLinkExist("user42_up"));
