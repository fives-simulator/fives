...
def plot_all_jobs(job_list):
    ...
    def to_sim_scale(timestamp, global_start, global_end):
        return (
            timestamp * ((full_width - 2 * PAD) / (global_end - global_start))
            + PAD
            - ((full_width - 2 * PAD) / (global_end - global_start)) * global_start
        )
    ...
    for job in job_list:
        ...
        d.append(
            draw.Lines(
                x1_job, current_job_offset, x2_job, current_job_offset, close=False, stroke="white", stroke_width=12
            )
        )
        d.append(
            draw.Text(
                f"JOB::{job['job_uid']} - {job['job_start_ts']:.6}s ->  {job['job_end_ts']:.6}s",
                font_size=10,
                x=x1_job + 4,
                y=current_job_offset + 4,
                font_family="Roboto",
                fill="#111111ff",
            )
        )
        ...
        for exec_id, actions in exec_sorted_actions.items():
            ...
            if exec_id not in [-1, -2]:
                ...
                d.append(
                    draw.Text(
                        f"R#{exec_id} - {action['act_start_ts']:.5}s",
                        font_size=12,
                        x=x1,
                        y=exec_y - 8,
                        font_family="Roboto",
                ...
                d.append(
                    draw.Lines(
                        ...
                        exec_y - 4,
                        x1,
                        exec_y - 4 + int(Y_OFFSET * len(actions) / 2),
                        close=False,
                        stroke="white",
                        stroke_width=1,
                        fill="white",
            ...
            for action in actions:
                ...
                if action["sub_job"] == job["job_uid"] and action["act_type"] == "CUSTOM":  # Custom action all sub jobs
                    ...
                    action["sub_job"] == job["job_uid"] and action["act_type"] == "SLEEP"
                ):  # Sleep action for reservation
...
def plot(job):
    ...
    def act_to_job_scale(timestamp, global_start, global_end):
        return (
            timestamp * ((WIDTH - 2 * PAD) / (global_end - global_start))
            + PAD
            - ((WIDTH - 2 * PAD) / (global_end - global_start)) * global_start
        )
    ...
    for action in sorted_rows:
        ...
        if action["sub_job"] == job["job_uid"] and action["act_type"] == "CUSTOM":
            ...
            if exec_id != current_exec:
                ...
                d.append(
                    draw.Text(
                        f"R#{exec_id} - {action['act_start_ts']}",
                        font_size=12,
                        x=x1,
                        y=current_y - 8,
                        font_family="Roboto",
        ...
        d.append(
            draw.Lines(
                x1, current_y, x2, current_y, close=False, stroke=color, stroke_width=Y_OFFSET - 2, stroke_opacity=0.8
            )
        )
        current_y += Y_OFFSET
