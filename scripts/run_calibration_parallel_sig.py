...
DATASET = os.getenv("CALIBRATION_DATASET", default="theta2022_fullMonth7_cat1")
DATASET_EXT = os.getenv("CALIBRATION_DATASET_EXT", default=".yaml")
BUILD_PATH = os.getenv("CALIBRATION_BUILD_PATH", default="../build")
CALIBRATION_RUNS = int(os.getenv("CALIBRATION_RUNS", default=100))
CFG_VERSION = os.getenv("CI_COMMIT_SHORT_SHA", default="0.0.1")
MAX_PARALLELISM = os.getenv("MAX_PARALLELISM", default=2)
...
def update_base_config(parametrization, base_config, cfg_name):
    ...
    # Lustre parameter, min_stripe_count (number of OST used for load-balancing parts of a file)
    # Overrides the read / write sc curve (see following params) minimum boundary
    if "min_stripe_count" in parametrization:
        min_stripe_count = parametrization.get("min_stripe_count")
        base_config["lustre"]["min_stripe_count"] = min_stripe_count

    # Parameters for the stripe_count (READ)
    if "read_sc_inflection_param" in parametrization:
        read_sc_inflection_param = parametrization.get("read_sc_inflection_param")
        base_config["lustre"]["read_sc_inflection_param"] = read_sc_inflection_param

    if "read_sc_rate_param" in parametrization:
        read_sc_rate_param = parametrization.get("read_sc_rate_param")
        base_config["lustre"]["read_sc_rate_param"] = read_sc_rate_param

    # Parameters for the stripe_count (WRITE)
    if "write_sc_inflection_param" in parametrization:
        write_sc_inflection_param = parametrization.get("write_sc_inflection_param")
        base_config["lustre"]["write_sc_inflection_param"] = write_sc_inflection_param

    if "write_sc_rate_param" in parametrization:
        write_sc_rate_param = parametrization.get("write_sc_rate_param")
        base_config["lustre"]["write_sc_rate_param"] = write_sc_rate_param

    # Parameters for the number of compute nodes involved in I/O (READ)
    if "read_nodes_inflection_param" in parametrization:
        read_nodes_inflection_param = parametrization.get("read_nodes_inflection_param")
        base_config["storage"]["read_nodes_inflection_param"] = read_nodes_inflection_param

    if "read_nodes_rate_param" in parametrization:
    ...
    if "write_nodes_rate_param" in parametrization:
        write_nodes_rate_param = parametrization.get("write_nodes_rate_param")
        base_config["storage"]["write_nodes_rate_param"] = write_nodes_rate_param

    # Parameters for the number of files used by each job (READ)
    if "read_files_inflection_param" in parametrization:
        read_files_inflection_param = parametrization.get("read_files_inflection_param")
        base_config["storage"]["read_files_inflection_param"] = read_files_inflection_param

    if "read_files_rate_param" in parametrization:
        read_files_rate_param = parametrization.get("read_files_rate_param")
        base_config["storage"]["read_files_rate_param"] = read_files_rate_param

    # Parameters for the number of files used by each job (WRITE)
    if "write_files_inflection_param" in parametrization:
        write_files_inflection_param = parametrization.get("write_files_inflection_param")
        base_config["storage"]["write_files_inflection_param"] = write_files_inflection_param

    if "write_files_rate_param" in parametrization:
        write_files_rate_param = parametrization.get("write_files_rate_param")
        base_config["storage"]["write_files_rate_param"] = write_files_rate_param
